...
class SimulatedStereo(DenseToSparse):
    ...
    def dense_to_sparse(self, rgb, depth):
        ...
        # Compute gradients using Sobel operator from scipy
        gx = sobel(blurred, axis=0, mode='constant')
        gy = sobel(blurred, axis=1, mode='constant')
        ...
        if self.dilate_iterations >= 0:
            kernel = np.ones((self.dilate_kernel, self.dilate_kernel), dtype=np.uint8)
            dilated_mask = self.dilate(mag_mask.astype(np.uint8), kernel)
        else:
            dilated_mask = mag_mask

        # Combine masks
        mask = np.bitwise_and(dilated_mask, depth_mask)
        return mask

    def dilate(self, mask, kernel):
        """ Perform dilation on the binary mask using the specified kernel. """
